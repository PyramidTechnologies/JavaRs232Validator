...
{
	private static final String ACTION_USB_PERMISSION =    "com.Rs232Validator.USB_PERMISSION";
	...
	public UARTLoopbackActivity.AndroidLogger _logger;

	/*constructor*/
	@SuppressLint({"UnspecifiedRegisterReceiverFlag", "UnspecifiedImmutableFlag"})
    public FT311UARTInterface(Context context, SharedPreferences sharePrefSettings, UARTLoopbackActivity.AndroidLogger logger){
		super();
	...
	/*read data*/
	public Tuple<Byte, byte[]> ReadData(int numBytes, byte[] buffer, int [] actualNumBytes){
		...
		if((numBytes < 1) || (totalBytes == 0)){
			actualNumBytes[0] = 0;
			status = 0x01;
			return new Tuple<>(status, buffer);
		}
		...
		return new Tuple<Byte, byte[]>(status, buffer);
	}
	...
	public byte[] Read(long l) {
		...
		while (status != 0x00) {
			...
			status = ReadData((int) l, readBuffer,actualNumBytes).x;
			//status--;
		}
		...
		//UARTLoopbackActivity.logger.LogTrace("Read %d bytes: %s", actualNumBytes[0], sb.toString());
	...
	public void Write(byte[] bytes) {
		...
		//_logger.LogTrace("Running Write() with %d bytes: %s", bytes.length, sb.toString());
