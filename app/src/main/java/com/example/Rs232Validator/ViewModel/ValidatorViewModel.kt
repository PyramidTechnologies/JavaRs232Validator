...
class ValidatorViewModel(application: Application) : AndroidViewModel(application){
    ...
    private val UsdBillValues = listOf(1, 2, 5, 10, 20, 50, 100)
    ...
    private fun registerListeners(){
        billValidator.OnBillStacked.addListener { args ->
            val billType = args.getOrNull(0) as? Int ?: return@addListener
            updateBillValue(billType)
        }

        billValidator.OnBillEscrowed.addListener { args ->
            billType = args.getOrNull(0) as? Int ?: return@addListener
            logger.LogInfo("Escrowed a bill of type %d.", billType)
            toggleIsBillInEscrow()
        }

        billValidator.OnStateChanged.addListener { args ->
            val state = args.getOrNull(0) as? StateChangedEventArgs ?: return@addListener

            logger.LogInfo("The state changed from %s to %s.", state.OldState.name, state.NewState.name)
            viewModelScope.launch {
                _currentState.value = state.NewState
            }
        }

        billValidator.OnBarcodeDetected.addListener { args ->
            val barcode: String = args.getOrNull(0) as? String ?: return@addListener
            logger.LogInfo("Detected a barcode: %s", barcode)
            viewModelScope.launch {
        ...
        billValidator.OnCashboxAttached.addListener {
            logger.LogInfo("The cashbox was attached.")
            viewModelScope.launch {
        ...
        billValidator.OnCashboxRemoved.addListener {
            logger.LogInfo("The cashbox was removed.")
            viewModelScope.launch {
        ...
        billValidator.OnEventReported.addListener { args ->
            val event = args.getOrNull(0) as? Rs232Event ?: return@addListener
        ...
        billValidator.OnConnectionLost.addListener {
            logger.LogInfo("Lost connection to the validator.")
        ...
        billValidator.OnCommunicationAttempted.addListener { args ->
            val event = args.getOrNull(0) as? CommunicationAttemptedEventArgs ?: return@addListener

            val entry = PayloadExchange(
                LocalDateTime.now().format(TimestampFormat),
                ByteUtils.ConvertToHexString(event.RequestMessage.payload, true, false),
                event.RequestMessage.toString(),
                ByteUtils.ConvertToHexString(event.ResponseMessage.payload, true, false),
                event.ResponseMessage.toString()
            )
    ...
    fun updateBillValue(billType: Int){
        viewModelScope.launch {
            if (billType >= 1 && billType <= UsdBillValues.size) {
                billValues[billType - 1]++
                billValues[7] += UsdBillValues[billType - 1]
                logger.LogInfo(
                    "Stacked a bill of type %d and added $%d to total",
                    billType,
                    UsdBillValues[billType - 1]
                )
            } else {
    ...
    private val _correctableComponent = MutableStateFlow("N/A")
    val correctableComponent = _correctableComponent.asStateFlow()
    ...
    fun ClearServiceFlags() {
        lateinit var component: CorrectableComponent
        when( _correctableComponent.value) {
            "Tach Sensor" -> component = CorrectableComponent.TachSensor
            "Bill Path" -> component = CorrectableComponent.BillPath
            "Cashbox Belt" -> component = CorrectableComponent.CashboxBelt
            "Cashbox Mechanism" -> component = CorrectableComponent.CashboxMechanism
            "MAS" -> component = CorrectableComponent.MAS
            "Spring Rollers" -> component = CorrectableComponent.SpringRollers
            "All" -> component = CorrectableComponent.All
            else -> {
                telemetryResponses[7] = "Invalid component selected"
                return
            }
        }

        viewModelScope.launch {
            val response = billValidator.ClearServiceFlags(component)?.await()
            val resultValue =
                if (response?.IsValid?.get() == true) "True" else ErrorMessage
    ...
    fun onCorrectableComponentChanged(newComponent: String) {
        _correctableComponent.value = newComponent
    }
    ...
    fun GetLastBarcode() {
        viewModelScope.launch {
            ...
            val resultValue: String
            if (response?.IsValid?.get() == true && response.barcode != "") {
                resultValue = response.barcode
            } else if (response?.IsValid?.get() == true) {
