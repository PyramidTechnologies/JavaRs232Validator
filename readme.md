...
This repo is an implementation of the RS-232 bill acceptor (A.K.A. validator) protocol for Android devices.

## Polling
Poll request messages are sent from the host (i.e. Android device) to the acceptor.
...
* 0x05 Get Service Usage Counter: Get the telemetry metrics since the last time an acceptor was serviced.
...
This library utilizes FTDI's FT311 UART Interface to communicate to the acceptor through a custom harness.
The FT311 chip is used as a bridge between the Android device and the acceptor while providing a 5 volt power supply to the Android device.
If the host is on an Android desktop device, the FT311 chip is not required and the `SerialPort` class can be replaced with a standard serial port implementation.
